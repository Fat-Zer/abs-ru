#!/bin/bash # rnd.sh: Outputs a 10-digit random number
...
# head: # -c4 option takes first 4 bytes.
...
with nothing (//), and if it succeeds, prints the result # ("p" is a flag to
the "s" command here, this is different #+ from the "p" command).
...
# In few word this sed commmand means: # "On the first line only, remove any
character up to the right-most space, #+ then print it."
